...
echo"<script>window.location='adminhome.php?view=add_service_mgr'</script>";
}
else
{
	 $s11=mysql_query("select * from service_mgr where cat_id='$j'");
if((mysql_num_rows($s11))>0)
{
echo"<script>alert('service manager Already Exists')</script>";	
echo"<script>window.location='adminhome.php?view=add_service_mgr'</script>";
}
else
{

 mysql_query("insert into service_mgr values('','$a','$c','$b','$g','$h','$un','$i','$j','1')");

mysql_query("insert into login values('','$un','$i','service manager','1')");
...
<body>
 <div class="templatemo_box">
   	    <h2 align="center">Add Service Manager</h2>
